...
#helm install --dry-run --debug ./aks-cluster-autoscaler --set nodes.min=$min_nodes,nodes.max=$max_nodes,nodes.agentpool=$agentPool,image.repository=$ca_image_repository,image.tag=$ca_image_tag --set rbac.create=false > debug.yaml
